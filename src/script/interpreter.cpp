        ...
        while (pc < pend)
        {
            ...
            switch (opcode)
            {
                ...
                case OP_CHECKSIGFROMSTACKVERIFY:
                {
                    ...
                    bool fSuccess = pubkey.Verify(hash, vchSig, true);
                    ...
                    if (!fIsLegacy) {
                        ...
                        while (fSuccess && nFlag > 0) {
                            ...
                            if (nFlag & 1) {
                                ...
                                if (!checker.CheckSig(vchSig, vchPubKey, scriptCode, sigversion, vscriptSigCode))
                                    return set_error(serror, SCRIPT_ERR_SIG_NULLFAIL);
                        ...
                        if (fSuccess && fUncoveredScriptSigCode) {
                            const uint32_t mask = ~((1U << posSigScriptCode) - 1);
                            fUncoveredScriptSigCode &= (uncovered | mask);
                        }
    ...
    if (witversion == 0) {
        ...
        if (witprogram.size() == 32) {
            ...
            if (IsMSStack(vPath, nPosition, stack, vchKeyCode) && vchKeyCode[0] == 0 && IsMSV0Stack(stack, vscriptSigCode))
                nSigOp = CScript(vchKeyCode.begin() + 1, vchKeyCode.end()).GetSigOpCount(true, true);
            else
                return nSigOp;
        }
        ...
        for (size_t i = 0; i < vscriptSigCode.size(); i++)
            nSigOp += vscriptSigCode[i].GetSigOpCount(true, true);
        return nSigOp;
    }
