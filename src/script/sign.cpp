...
bool TransactionSignatureCreator::CreateSig(std::vector<unsigned char>& vchSig, const CKeyID& address, const CScript& scriptCode, SigVersion sigversion) const
{
    ...
    uint256 hash = SignatureHash(scriptCode, vscriptSigCode, *txTo, nIn, nHashType, amount, nFees, sigversion);
                               ...
                               const vector<valtype>& sigs1, const vector<valtype>& sigs2, SigVersion sigversion)
{
    ...
    BOOST_FOREACH(const valtype& sig, allsigs)
    {
        for (unsigned int i = 0; i < nPubKeys; i++)
        {
            ...
            if (checker.CheckSig(sig, pubkey, scriptPubKey, sigversion, vscriptSigCode))
            {
                sigs[pubkey] = sig;
                break;
            }
